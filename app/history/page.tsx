      ...
      <main className="relative z-10 max-w-2xl mx-auto px-5 py-20">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold mb-5"
            style={{ background: "rgba(0,163,255,0.1)", border: "1px solid rgba(0,163,255,0.25)", color: "#00A3FF" }}>
          ...
          <p className="text-slate-400 text-base leading-relaxed max-w-md mx-auto">
            Enter the email you used when submitting a video. We&apos;ll show all your past AI processing jobs and any clips we found.
          </p>
        </div>
            ...
            {jobs.length === 0 ? (
              <div className="rounded-2xl p-10 text-center"
                ...
                <p className="text-slate-500 text-sm mb-6">
                  No processing jobs were found for <span className="text-slate-300">{email}</span>.
                ...
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 rounded-xl font-bold text-sm text-slate-300"
              ...
              <div className="flex flex-col gap-4">
                <p className="text-slate-500 text-sm">
                  Found <span className="text-white font-bold">{jobs.length} reel{jobs.length !== 1 ? "s" : ""}</span> for {email}
                </p>
                {jobs.map((job) => {
                  ...
                  return (
                    <div key={job.id} className="rounded-2xl p-5"
                      style={{ background: "#0A1628", border: "1px solid rgba(255,255,255,0.08)" }}>
                      <div className="flex items-start justify-between gap-4 mb-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap mb-1">
                            <span className="font-bold text-white text-sm">
                              {[job.first_name, job.last_name].filter(Boolean).join(" ") || "Unknown"}
                            </span>
                            {job.jersey_number !== null && (
                              <span className="font-black text-[#00A3FF] text-sm">#{job.jersey_number}</span>
                            )}
                            {job.sport && (
                      ...
                      {isFailed && job.error_message && (
                        <p className="text-xs text-red-400 mb-3 px-3 py-2 rounded-lg"
                      ...
                      {isActive && (
                        <p className="text-xs text-slate-500 mb-3">
                          Still processing — check back soon or refresh this page.
                        </p>
                      )}
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                        {isComplete && clipCount > 0 && (
                          <button
                            ...
                            style={{ background: "linear-gradient(135deg, #0055EE, #00A3FF)", boxShadow: "0 0 20px rgba(0,120,255,0.3)" }}>
                            Load These Clips →
                          </button>
                        )}
                        {isFailed && (
        ...
        {!searched && (
          <div className="text-center text-slate-600 text-sm mt-8">
            <p>Previously submitted a video? Your clips are stored by email.</p>
            <p className="mt-1">
              New here?{" "}
              <button onClick={() => router.push("/process")} className="text-[#00A3FF] hover:underline">
